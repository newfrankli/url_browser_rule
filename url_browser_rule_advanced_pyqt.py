...
if hasattr(sys, '_MEIPASS'):
    # 打包后的临时目录
    ICON_FILE = os.path.join(sys._MEIPASS, 'url.ico')
else:
    # 开发环境下的当前目录
...
class TransparentWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 对于打包后的exe，使用sys.executable作为路径，而不是脚本路径
    ...
    def register_protocol(self):
        ...
        try:
            ...
            with winreg.CreateKey(winreg.HKEY_CURRENT_USER, command_path) as key:
                # 对于打包后的exe，直接使用sys.executable运行，不需要pythonw.exe
                command = f'"{self.script_path}" "%1"'
    ...
    def setup_appearance_tab(self, parent):
        ...
        self.opacity_slider.setValue(int(self.config.get('opacity', 1.0) * 100))
        layout.addWidget(self.opacity_slider, 2, 1)
        
        self.opacity_label = QLabel(f"{self.config.get('opacity', 1.0):.1f}")
    ...
    def import_rules(self):
        ...
        file_btn.clicked.connect(lambda: self.import_from_file(text_edit))
        ...
        if dialog.exec_():
            urls = text_edit.toPlainText().split('\n')
            browser = browser_combo.currentText()
            self.process_imported_urls(urls, browser)
    
    def import_from_file(self, text_edit):
        """从文件导入URL列表"""
    ...
    def process_imported_urls(self, urls, browser):
        """处理导入的URL列表"""
        success_count = 0
        failed_count = 0
            ...
            if url:
                ...
                try:
                    ...
                    if parsed_url.netloc:
                        pattern = parsed_url.netloc
                    else:
                        # 处理没有协议的URL
                        if '.' in url:
                            pattern = url.split('/')[0].split(':')[0]
                        else:
                            continue
                    ...
                    exists = False
                    for rule in self.rules:
                        if rule["pattern"] == pattern and rule["browser"] == browser:
                            exists = True
                            break
        ...
        if success_count > 0:
            self.save_rules(self.rules)
            self.load_rules_to_tree()
            QMessageBox.information(self, "成功", f"导入完成！\n成功: {success_count} 条\n已存在: {failed_count} 条")
        else:
            QMessageBox.warning(self, "警告", "没有导入任何新规则")
    ...
    def save_appearance_settings(self):
        ...
        self.config['border_thickness'] = self.border_spin.value()
    ...
    def restore_default_scaling(self):
        ...
        self.resize_widgets()
        self.save_config()
        QMessageBox.information(self, "成功", "缩放比例已恢复默认值！")

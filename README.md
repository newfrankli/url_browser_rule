...
- 🎨 基于PyQt5的高级GUI界面
- 👻 透明输入框设计，深色/透明背景
- 📋 可视化规则管理（查看、添加、编辑、删除、批量导入/从文件导入）
...
- 📤 支持批量导入URL规则
- 🎨 外观设置：可调整字体类型、字体大小、窗口透明度、边框厚度
- 🔄 支持恢复默认缩放
- 📁 支持从文件导入规则
- 🔍 浏览器路径智能查找（优先注册表，其次系统PATH，最后常见路径）
- 🚀 浏览器路径预扫描线程，提高响应速度
- 🔒 安全的URL执行方式，防止命令注入
...
2. **调整外观**：
   - **字体类型**：从下拉列表中选择系统可用的字体
   - **字体大小**：通过数值框调整字体大小（8-24）
   - **窗口透明度**：通过滑块调整窗口透明度（10%-100%）
   - **边框厚度**：通过数值框调整边框厚度（1-5）

3. **保存设置**：
   - 点击"保存外观设置"按钮应用更改
   - 点击"恢复默认缩放"按钮可恢复默认缩放比例
...
├── URLBrowserRule.spec                # PyInstaller打包配置文件
├── build/                             # PyInstaller构建目录
└── dist/                              # PyInstaller生成的可执行文件目录
```
...
### v3.1.0（最新版本）
- 🎨 新增外观设置功能：可调整字体类型、字体大小、窗口透明度、边框厚度
- 📁 支持从文件导入规则
- 🔄 新增恢复默认缩放功能
- 🚀 新增浏览器路径预扫描线程，提高响应速度
- 🔍 优化浏览器路径智能查找算法
